...
from .db import conn, get_setting, init_db
from .routers import bodyops, vaultflow, webhooks
...
def today():
    ...
    with conn() as c:
        ...
        steps_row = c.execute(
            "SELECT count FROM steps WHERE date=?", (date.today().isoformat(),)
        ).fetchone()
        vit_row = c.execute(
            "SELECT taken FROM vitamins WHERE date=?", (date.today().isoformat(),)
        ).fetchone()
